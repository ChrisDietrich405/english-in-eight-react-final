export default function AdjectiveDefinition() {
    return (
    <>
    <div className="page-title">
        <h2>Adjectives Definition</h2>
    </div>
    <div className="expressions-definition-body">
        <p> An adjective is a word that tells us more about a noun. It "describes" a noun. In these examples, the adjectives are in bold and the nouns are in italics.</p>
    </div>
    <div className="definition-pictures-container">
                <div className="definition-pictures">
                    <img src="../../../images/strong-man-adjectives-definition.jpg" alt=""/>
                    <p><strong>Strong</strong> <em>man</em></p>  
                </div>
                <div className="definition-pictures">
                    <img src="../../../images/happy-child-adjectives-definition.jpg" alt=""/>
                    <p><strong>Happy</strong> <em>child</em></p> 
                    
                </div>
                <div className="definition-pictures">
                    <img src="../../../images/big-dog-adjectives-definition.jpg" alt=""/>
                    <p><strong>Big</strong> <em>dog</em></p> 
                    
                </div>
            </div>
        </>
    )
}